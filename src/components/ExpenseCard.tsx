...
export const ExpenseCard: React.FC<ExpenseCardProps> = ({ expense, onEdit, onDelete, formatAmount }) => {
  ...
  return (
    <article
      className="flex w-full flex-col gap-3 rounded-2xl border border-white/10 bg-slate-950/50 p-4 text-slate-100 shadow-[0_12px_30px_rgba(15,23,42,0.35)] transition hover:-translate-y-0.5 hover:bg-slate-900/60"
    ...
    >
      <div className="flex flex-1 flex-col gap-1">
        ...
        {expense.note.length > 0 ? <p className="text-sm text-slate-300">{expense.note}</p> : null}
      </div>
      <div className="flex flex-col items-end justify-between gap-3 text-xs text-slate-500 sm:flex-row sm:items-center">
